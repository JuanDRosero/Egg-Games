...
var rbola=40;
var tamJ = 80;
var imgJugador, imgOponente, imgBola;
var canvas, ctx;
var score = [0, 0];
var jugando = false;
function cargaImagenes() {
    imgJugador = new Image();
    imgOponente = new Image();
    imgBola = new Image();

    imgJugador.src = 'Recursos/jugador.png';
    imgOponente.src = 'Recursos/oponente.png';
    imgBola.src = 'Recursos/pelota.png';
}
        ...
        if (jugando == true) {
            switch (direccion) {
                case 1:
                    ...
                    break;
                case -1:
                    if (this.posy + this.y < alto - altutaJ) {
                        this.posy += this.vy;
                    }
                    break;
            }
...
function principal() {
    if (jugando == false) {
        setTimeout(function () {
            jugando = true;
            dibujarMarcador();
            Ball.inicio(Math.floor(Math.random() * (3 - 1)) + 1, Math.floor(Math.random() * (3 - 1)) + 1);
            jugando = true;
        }, 1500);
    }
    if (contador % 10000 == 0) {
